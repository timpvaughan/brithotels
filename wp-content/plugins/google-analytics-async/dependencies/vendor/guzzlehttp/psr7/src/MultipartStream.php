...
class MultipartStream implements \Beehive\Psr\Http\Message\StreamInterface
{
    ...
    protected function createStream(array $elements)
    {
        ...
        $stream->addStream(\Beehive\GuzzleHttp\Psr7\Utils::streamFor("--{$this->boundary}--\r\n"));
        return $stream;
    }
    private function addElement(\Beehive\GuzzleHttp\Psr7\AppendStream $stream, array $element)
    {
        ...
        $element['contents'] = \Beehive\GuzzleHttp\Psr7\Utils::streamFor($element['contents']);
        ...
        $stream->addStream(\Beehive\GuzzleHttp\Psr7\Utils::streamFor($this->getHeaders($headers)));
        $stream->addStream($body);
        $stream->addStream(\Beehive\GuzzleHttp\Psr7\Utils::streamFor("\r\n"));
    }
    /**
     * @return array
     */
    private function createElement($name, \Beehive\Psr\Http\Message\StreamInterface $stream, $filename, array $headers)
    {
        ...
        if (!$type && ($filename === '0' || $filename)) {
            if ($type = \Beehive\GuzzleHttp\Psr7\MimeType::fromFilename($filename)) {
