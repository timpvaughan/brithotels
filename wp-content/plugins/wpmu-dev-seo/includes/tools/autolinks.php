...
class Smartcrawl_Autolinks extends Smartcrawl_Base_Controller { // phpcs:ignore -- We have two versions of this class
	...
	private $backup_fragments = array();
	...
	private function backup_fragments( $text, $exclude_headings ) {
		$this->backup_fragments = array();
	...
	private function restore_fragments( $text ) {
		return str_replace(
			array_keys( $this->backup_fragments ),
			array_values( $this->backup_fragments ),
			$text
		);
	}
	...
	private function replace_fragment_with_placeholder( $matches ) {
		...
		$this->backup_fragments[ $placeholder ] = $fragment;
	...
	private function process_text( $text, $mode ) {
		...
		if ( is_feed() && ! $allow_feed ) {
			return $text;
		} elseif ( ! empty( $options['onlysingle'] ) && ! ( is_single() || is_page() ) ) {
			return $text;
		}
		...
		$text = $this->backup_fragments(
			$text,
			! empty( $options['excludeheading'] )
		);
		...
		if ( ( ! empty( $post->post_type ) && ! empty( $options["{$post->post_type}"] ) ) ) {
			...
			foreach ( $posts as $postitem ) {
				if ( (int) $postitem->ID === (int) $post->ID ) {
					continue;
				}
				if (
					...
					( get_permalink( $postitem->ID ) !== $thisurl ) &&
