...
class Smartcrawl_Schema_Source_Woocommerce extends Smartcrawl_Schema_Property_Source {
	...
	public function get_value() {
		...
		switch ( $this->field ) {
			...
			case self::GLOBAL_ID:
				return $this->get_global_identifier();
	...
	private function get_global_identifier() {
		...
		$global_id_key = (string) smartcrawl_get_array_value( $options, 'global_identifier' );
		if ( ! $global_id_key ) {
			return '';
		}

		return $this->product->get_meta( Smartcrawl_Controller_Woo_Global_Id::GLOBAL_ID_META_KEY );
	}
}